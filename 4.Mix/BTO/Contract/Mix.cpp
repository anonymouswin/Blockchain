...
bool Coin::mint_m(int t_ID, int receiver_iD, int amount, int * time_stamp) 
{
	if(t_ID != minter)
	{
		cout<<"\nERROR:: Only ''MINTER''can initialize Accounts\n";
		return false;//false not a minter.
	}
	...
	*time_stamp          = T->TID;      //return time_stamp to user.
	...
	int rv_i = lib->read(T, rec_obj);
	if(rv_i != SUCCESS)//read into local tob for sender_iD.
	{
		return false;//AU aborted.
	}
	*(int*)rec_obj->value = *(int*)rec_obj->value + amount;
	
	rv_i = lib->write(T, rec_obj);

	rv_i = lib->try_commit(T, error_id);
	if(rv_i == SUCCESS)	
	{
		return true;//AU execution successful.
	}
...
int Coin::send_m(int t_ID, int sender_iD, int receiver_iD, int amount,
									 int *time_stamp, list<int>&conf_list) 
{
	...
	*time_stamp            = T->TID;      //return time_stamp to user.
	...
	int rv_s = lib->read(T, sender_obj );
	int rv_r = lib->read(T, rec_obj);
	if(rv_s != SUCCESS || rv_r != SUCCESS) 
		return 0;//AU aborted.
	...
	*(int*)rec_obj->value     = *(int*)rec_obj->value + amount;
	...
	rv_s = lib->write(T, sender_obj);
	rv_r = lib->write(T, rec_obj);
	
	int rv_tryC = lib->try_commit_conflict(T, error_id, conf_list);
	if(rv_tryC == SUCCESS ) 
	{
