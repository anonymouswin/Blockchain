...
//		cGraph->print_grpah();
	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		Timer Ttimer;
		 ...
		 //! flag is used to add valid 
		 //! AUs in Graph (invalid AU: 
		 //! senders does't have 
		...
		//! statrt clock to get 
		//! time taken by this transaction.
		...
		while(curInd < numAUs)
		{
			...
			if (flag == true)
			{
				...
				//! IMP::delete time stamps in
				//! conflict list, which are added
				//! because of initilization 
				//! version by mnit() trycommit.
...
public:
	...
	void mainValidator()
	{
		...
		//!fixed ammount; mint_val() function
		//! is assume to be serial.
		for(int sid = 1; sid <= SObj; sid++) 
		{
			//! 0 is contract deployer.
			bool r = coin->mint(0, sid, bal);
			total  = total + bal;
		}
	...
	static void concValidator( int t_ID )
	{
		...
		while( true )
		{
			...
			{				
				...
				if(temp->in_count == 0)
				{
					...
					if(temp->marked == false )
					{
						...
						//! again:: even mark field help
						//! but this is optimization 
						...
						//! get the AU to execute, which 
						//! is of string type; listAUs 
						//! index statrt with 0.
			...
			//! reached to end of local buffer;
			//! clear the buffer.
			buffer.clear();
...
int main( )
{
	...
	for(int numItr = 0; numItr < totalRun; numItr++)
	{
		...
		//! index+1 represents respective 
		//! AU id, & mAUT[index] represents 
