	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		...
		while(curInd < numAUs)
		{
			...
			if(tmp.compare("send") == 0)
			{
				...
				{
					aCount[t_ID]++;
					v = coin->send_m(t_ID, s_id, r_id, amt,
										 &t_stamp, conf_list);
					if(v == -1)
					{
...
int main( )
{
	...
	{
		 //! generates AUs (i.e. trans 
		 //! to be executed by miner & validator).
...
//			<<numItr+1<<" Over =====================\n"<<endl;
	...
	//to get total avg miner and validator time after number of totalRun runs.
