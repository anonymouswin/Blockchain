...
|  *****     File:   lock_free.cpp                           *****  |
|  *****  Created:   on Aug 18, 2018, 02:15 AM               *****  |
*********************************************************************
...
class Miner
{
	...
	Miner(int minter_id)
	{
		...
		//! used to execute the numAUs to 0.
		currAU     = 0;
		
		//! initialize the graph node counter to
		//! 0 (number of AUs added in graph,
	...
	void mainMiner()
	{
		...
		//! amount; mint() is assume to be serial.
	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		...
		while(curInd < numAUs)
		{
			...
			if(tmp.compare("get_bal") == 0)
			{
				...
				bool v = coin->get_bal_m(s_id, &bal, 
											t_ID, &t_stamp, conf_list);
				while(v == false)  //! execute again if tryCommit fails.
				{
					aCount[t_ID]++;
					v = coin->get_bal_m(s_id, &bal, 
											t_ID, &t_stamp, conf_list);
				}
			...
			if (flag == true)
			{
				...
				{
					int i = 0;
					//! find the conf_AU_ID in 
					//! map table given conflicting
					//! time-stamp.
					while(*it != mAUT[i])
						i = (i+1)%numAUs; 
...
public:
	Validator()
	{
		...
		//! represents respective thread id.
...
//		finalState();
	...
	static void concValidator( int t_ID )
	{
		...
		while( true )
		{
			//! uncomment this to remove the effect
...
//			buffer.clear();
			...
			while(verTemp != cGraph->verTail)
			{
				...
				{
					//! expected in_degree is 0 then vertex
					//! can be executed if not claimed by other thread.
					int expected = 0;
					if(atomic_compare_exchange_strong(
