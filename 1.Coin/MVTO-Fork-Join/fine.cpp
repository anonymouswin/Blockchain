/********************************************************************
|  *****     Coin Contract MVTO Miner + Fork-Join Validator  *****  |
|  *****     File:   fine.cpp                                *****  |
...
//		FILEOPR file_opr;
//		file_opr.print_Map_AUTrns(mAUT, numAUs);        //! print AU_ID and Timestamp.
...
public:
	...
	void mainValidator()
	{
		...
		//! initialization of account with fixed ammount; mint_val() function is assume to be serial.
		for(int sid = 1; sid <= SObj; sid++) 
		{
		...
		//!----------------------------------------------------
		//! MASTER THREAD CREATE n VALIDATOR WORKER THREADS   !
		//!----------------------------------------------------
...
//		finalState();                                 //! print the final state of the shared objects by validator
	...
	{
		Timer Ttimer;
		auto start = Ttimer._timeStart();               //! start timer to get time taken by this thread.
