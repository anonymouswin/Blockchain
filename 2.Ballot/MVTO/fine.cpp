/********************************************************************
|  *****    Ballot Contract MVTO Miner Validator             *****  |
|  *****     File:   fine.cpp                                *****  |
...
//		cout<<"!!!!!!!   Fine-Grain Miner Thread Start           !!!!!!\n";
			...
			for(itr = buffer.begin(); itr != buffer.end(); itr++)
			{
				...
				if(temp->in_count == 0)
				{
					...
					if(temp->marked == false )
					{
						...
						//! this node will not be executed again:: even mark field 
						//! help but this is optimization kind of logical delete.
						temp->in_count = -1;

						//! num of Valid AUs executed is eAUCount+1.
						eAUCount++;
						
						//! relase lock X1 either here or in last else condition.
