/********************************************************************
|  *****    Ballot Contract MVTO Miner + Fork-Join Validator *****  |
|  *****     File:   coarse.cpp                              *****  |
	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		...
		while( curInd < numAUs )
		{
			...
			if(tmp.compare("vote") == 0)
			{
				...
				int v = ballot->vote_m(vID, pID, 
											&t_stamp, conf_list);
				while( v != 1 )
				{
					aCount[t_ID]++;
					v = ballot->vote_m(vID, pID,
										&t_stamp, conf_list);					
					if(v == -1)
					{
			...
			if(tmp.compare("delegate") == 0)
			{
				...
				int v = ballot->delegate_m(sID, rID, &t_stamp, 
														conf_list);	
				while( v != 1 )
				{
					aCount[t_ID]++;
...
int main( )
{
	...
	//! read from input file:: nProposal = 
